...
async def apply_manual_timestamps_to_file(timestamps: str, audio_file: str):
    ...
    ffmpeg_cmd = f'ffmpeg -i "{audio_file}" -i {metadata_file} -map_chapters 1 -map_metadata 1 -c copy -y "{audio_file}.tmp.{FILE_EXTENSION}" && mv "{audio_file}.tmp.{FILE_EXTENSION}" "{audio_file}"'
